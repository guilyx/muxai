...
func TestRunSuccess(t *testing.T) {
	p := NewProvider(
		WithCommand("cursor"),
		WithRunner(&fakeRunner{out: " result "}),
	)
...
func TestRunClassifiesAuthError(t *testing.T) {
	p := NewProvider(
		WithCommand("cursor"),
		WithRunner(&fakeRunner{err: errors.New("unauthorized")}),
	)
	...
	if !muxai.IsCode(err, muxai.ErrorCodeAuth) {
		t.Fatalf("expected auth code, got %v", err)
	}
}
